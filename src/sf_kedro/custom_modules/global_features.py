...
class MarketLogReturnFeature(sf.feature.GlobalFeature):
    ...
    outputs: ClassVar[list[str]] = ["market_log_ret"]
    ...
    def compute(self, df: pl.DataFrame, context: dict[str, Any] | None = None) -> pl.DataFrame:
        ...
        df = df.with_columns(pl.col(self.price_col).log().diff().over(self.group_col).alias("__log_ret"))

        market_ret = df.group_by(self.ts_col).agg(pl.col("__log_ret").mean().alias("market_log_ret"))
