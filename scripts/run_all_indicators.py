...
def get_all_indicator_classes() -> list[tuple]:
    ...
    for registry_name in feature_names:
        try:
            ...
            class_obj = sf.default_registry.get(component_type=sf.SfComponentType.FEATURE, name=registry_name)
...
def update_parameters_file(registry_name: str, params: dict[str, Any], output_feature_name: str) -> None:
    """
    ...
    config["feature_analysis"]["features"]["extractors"] = [create_feature_config(registry_name, params)]
...
def generate_consolidated_report(all_stats: list[dict[str, Any]], output_file: Path) -> None:
    ...
    lines.append("=" * 120)
    header = (
        f"{'INDICATOR':<35} | {'MEAN':>10} | {'STD':>10} | {'SKEW':>7}"
        f" | {'KURT':>7} | {'CORR_AVG':>10} | {'VALID%':>8} | {'TRANSFORM?':<12}"
    )
    lines.append(header)
    lines.append("=" * 120)
    ...
    for stat in sorted_stats:
        ...
        row = (
            f"{indicator:<35} | {mean:>10.6f} | {std:>10.6f} | {skew:>7.3f}"
            f" | {kurt:>7.3f} | {corr_str:>10} | {valid_pct:>7.2f}% | {transform:<12}"
        )
        lines.append(row)
    ...
    pct = transform_needed / len(all_stats) * 100
    lines.append(f"Indicators needing transformation: {transform_needed} / {len(all_stats)} ({pct:.1f}%)")
